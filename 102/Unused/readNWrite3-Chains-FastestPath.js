...
const init = async () => {
  ...
  for (let i = +startNo; i < +stopNo + 1; i++) {
    ...
    const makeRequest = async () => {
      const promise1 = await contractAccess102.methods.getAllRecords(i).call({ from: coinbaseAddr102 });
      const promise2 = await contractAccess103.methods.getAllRecords(i).call({ from: coinbaseAddr103 });
      const promise3 = await contractAccess104.methods.getAllRecords(i).call({ from: coinbaseAddr104 });

      const [response1, response2, response3] = await Promise.race([promise1, promise2, promise3]);
      return response1.ok ? response1 : (response2.ok ? response2 : response3);
    }

    // console.log('<<==========================<<   102   >>==========================>>');
    const startReadingTime = new Date().getTime();
    console.log(`Start Reading TimeStamp: ${startReadingTime}\n`);
